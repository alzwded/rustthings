...
fn create_pool(num_threads: usize) -> Result<rayon::ThreadPool, String> {
    match rayon::ThreadPoolBuilder::new()
        .num_threads(num_threads)
        .build()
    {
        Err(e) => Err(format!("Failed to build thread pool: {}", e)),
        Ok(pool) => Ok(pool),
    }
}
...
pub fn downscale(img: &DynamicImage, new_res: &resolution::Resolution) -> Result<DynamicImage, String> {
    ...
    let rawsamples: Vec<u8> = create_pool(2)?.install(|| -> Vec<u8> {
        (0..olen as u32).into_iter()
            .map(|i| -> WorkItem<_> {
                WorkItem { 
                    iflat: &iflat, 
                    tx: i % new_res.width(),
